...
// For adding mock data during development
export function addMockCriticisms(projectRoot: string): void {
  const { addCriticism, generateCriticismId } = require("../lib/criticism/store");

  const mocks: Omit<Criticism, "id" | "createdAt">[] = [
    {
      category: "ELIM",
  ...
  for (const mock of mocks) {
    const id = generateCriticismId(mock.category, mock.subject, mock.files);
    addCriticism(projectRoot, {
      ...mock,
      id,
